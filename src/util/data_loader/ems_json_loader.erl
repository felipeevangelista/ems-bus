...
handle_info(check_sync_full, State = #state{name = Name,
	...
	case Hour >= 4 andalso Hour =< 6 of
		true ->
			ems_logger:info("~s force load checkpoint.", [Name], LogShowDataLoaderActivity),
...
handle_info({_Pid, {error, Reason}}, State = #state{name = Name,
													update_checkpoint = UpdateCheckpoint,
													log_show_data_loader_activity = LogShowDataLoaderActivity}) ->
	ems_logger:warn("~s is unable to load or update data. Reason: ~p.", [Name, Reason], LogShowDataLoaderActivity),
	{noreply, State, UpdateCheckpoint};
			...
			{ok, Records} ->
				case do_clear_table(State) of
					ok ->
						...
						ok;
					Error ->
						ems_logger:error("~s could not clear table before load data.", [Name], LogShowDataLoaderActivity),
						Error
				end;
			Error4 -> 
				ems_logger:warn("~s does not load data from filesystem.", [Name], LogShowDataLoaderActivity),
				Error4
		end
	catch
		_Exception:Reason3 -> 
			...
			{ok, Records} ->
				...
				case InsertCount > 0 orelse UpdateCount > 0 orelse ErrorCount > 0 orelse DisabledCount > 0 of
					true ->
						...
						ems_logger:info("~s sync ~p inserts, ~p updates, ~p disabled, ~p skips, ~p errors since ~s.", [Name, InsertCount, UpdateCount, DisabledCount, SkipCount, ErrorCount, LastUpdateStr], LogShowDataLoaderActivity);
					false -> ok
				end,
				ok;
			{error, Reason} = Error -> 
				ems_logger:error("~s update data error: ~p.", [Name, Reason], LogShowDataLoaderActivity),
				Error
		end
	catch
		_Exception:Reason3 -> 
