...
-export([read_private_key/1, read_public_key/1, read_certificate/1 ,read_pdf/1, sign_document/1, verify_sign/3, verify_document/1]).
...
sign_document(Request) -> 
    ...
    read_certificate("/home/rcarauta/desenvolvimento/certificado/client.crt"),
    PDF = read_pdf("/home/rcarauta/Downloads/file.pdf"),
    read_all_files_path("/home/rcarauta/desenvolvimento/certificado/autoridades"),
    sign_pdf(PrivateKey, PDF),

    {ok, Request#request{code = 200,
            content_type_out = <<"application/json">>,
            response_data = <<"{\"response:\" \" Documento Assinado!\"}">>}
	}.
...
verify_document(Request) ->
    ...
    PDF = read_pdf("/home/rcarauta/Downloads/file.pdf"),
    case verify_sign(PDF, "/home/rcarauta/Downloads/file-copia.pdf", PublicKey) of
        true -> {ok, Request#request{code = 200,
                    content_type_out = <<"application/json">>,
                    response_data = <<"{\"response:\" \" Documento Válido!\"}">>}};
        false -> {ok, Request#request{code = 200,
                      content_type_out = <<"application/json">>,
                      response_data = <<"{\"response:\" \" Documento Inválido!\"}">>}}
    end.
...
sign_pdf(PrivKey, Msg) ->
    ...
    file:write_file("/home/rcarauta/Downloads/file-copia.pdf", SigBin, [append]).
