%%********************************************************************
%% @title Module ems_api_query_postgresql
%% @version 1.0.0
...
-module(ems_api_query_postgresql).
...
find(FilterJson, Fields, Limit, Offset, Sort, Datasource) ->
	case ems_api_query_postgresql_parse:generate_dynamic_query(FilterJson, Fields, Datasource, Limit, Offset, Sort) of
		{ok, {Sql, Params}} -> 
...
find_by_id(Id, Fields, Datasource) ->
	case ems_api_query_postgresql_parse:generate_dynamic_query(Id, Fields, Datasource) of
		{ok, {Sql, Params}} -> execute_dynamic_query(Sql, Params, Datasource);
		{error, Reason} -> {error, Reason}
	end.

delete(Id, Datasource) ->
	case ems_api_query_postgresql_parse:generate_dynamic_delete(Id, Datasource) of
		{ok, {Sql, Params}} -> execute_dynamic_query(Sql, Params, Datasource);
		{error, Reason} -> {error, Reason}
	end.
...
execute_dynamic_query(Sql, Params, Datasource) ->
	try
		?DEBUG("ems_api_query_postgresql execute sql: ~s. Params: ~p.", [Sql, Params]),
